...
const I_9: React.FC<SecondPageProps> = ({ onNext, onPrevious, context ,_sp }) => {
 ...
 const [UploadError, setUploadError] = React.useState('')
  ...
  //         ...preve,
  //         I_9: { getimageI_9: reader.result as string }
  //       }))
  ...
  const uploadFile = async (e: React.FormEvent) => {
    e.preventDefault();
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            //   // if(formData.I_9.getimageI_9){
            //   // fetchPdf(formData.registration.fullName, context, "I_9")
            //   //   .then()
    ...
    <form onSubmit={uploadFile}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        ...
        <PrimaryButton onClick={async() => {
          await DownloadFileFromSharepointLibrary(_sp ,context,"RFCMasterDocuments","I_9.pdf");
          // await DownloadFileFromSharepointLibrary(_sp ,context,'aman',"WI4.pdf");
        }}>Download (I-9) Document</PrimaryButton>
          ...
          }} > <span>Upload (I-9)Document</span></IconButton>
          ...
          <Stack verticalFill style={{display:'flex', alignItems :'center', justifyContent:"flex-end"}}>
            ...
            <PrimaryButton type="submit"  style={{ margin: 0 }}>Next</PrimaryButton>
          </Stack>
        </div>
      </div>
            </form>
